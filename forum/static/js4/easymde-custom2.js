...
var BBCodeQuoteRenderer = {
    ...
    stripOutUnwantedNewLineChars: function(plainText) {    
        ...
        strippedText = plainText.replace(rThreeNewLineChars, '\n\n')
    ...
    registeChangeEvent: function() {
        ...
        editor.codemirror.on('change', function(instance, changeObj) {
            ...
            if (MentionRenderer.shouldFetchEnteredMentionObjList) {
                ...
                MentionObjLab.fetchMentionObjListInEditor(usernameList, self.onFetchSuccess) 
            }
        })
    },
...
var MentionObjLab = {
    ...
    addMentionObj: function(mentionObj) {
        if (mentionObj && mentionObj.username 
...
var MentionDropdown = {
    ...
    onUpKeyPressed: function() {
        ...
        var activeClass = self.activeDropdownItemSelector
                            .replace('.', '')
        var $activeItem = self.$dropdown
                            .find(self.activeDropdownItemSelector);
        ...
        if ($activeItem.index() > 0) {
            ...
            $activeItem.toggleClass(activeClass);
            $dropdownItem.eq($dropdownItem.length - 1)
    ...
    setDropdown: function (newMentionList) {
        ...
        activeClass = 
                self.activeDropdownItemSelector.replace('.', '');
        $(self.dropdownItemSelector).eq(0).addClass(activeClass);
        self.postionDropDown();
    ...
    appendDropdownItem: function(mention) {
        ...
        var strongStartWithHtml = '<strong>' + this.startWith + '</strong>'
        var text = mention['username']
                    .replace(startWithRegex, strongStartWithHtml)
            
        var dropdownItem = '<li class="username-item"><a href="' + mention['profile_url'] + '">' 
        + '<img src="' + mention['avatar_url'] + '"><span>' + text + '</span></a></li>' ;
    ...
    postionDropDown: function() {
        ...
        var top = this.codemirror.cursorCoords().top 
                    - this.$dropdown.height() - 3;
    ...
    onMentionItemClicked: function(username, profileURL) {
        ...
        var mentionObj = { username: username, profileURL: profileURL }
        MentionObjLab.addMentionObj(mentionObj);
        var mentionPlusSpace = textBeforeCursorPos.replace(this.pattern,  mention + ' ')
