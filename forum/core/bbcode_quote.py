...
def replace(match):
    result = re.search(quote_placeholder_start_tag_regex,
                       match.group(2), re.DOTALL).groupdict()
...
def bbcode_quote(text):
    placeholder_start_tag_count = len(re.findall(
        quote_placeholder_start_tag_regex, text))
    ...
    tag_length = start_tag_total_count if (
        start_tag_total_count <= end_tag_count) else end_tag_count

    for count in range(tag_length):
        placeholder_start_tag_test = len(re.findall(
            quote_placeholder_start_tag_regex, text)) > 0
